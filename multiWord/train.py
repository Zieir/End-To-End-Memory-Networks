...
def train_multiword(task_id, epochs=100, batch_size=32, max_ans_len=5,
    ...
    # reduction='sum' to match paper protocol (cost not averaged over a batch)
    ...
    for epoch in range(epochs):
        ...
        for i in range(0, len(Y_tr), batch_size):
            optimizer.zero_grad()
            logits = model(Xs_tr[i:i+batch_size], Xq_tr[i:i+batch_size])
            loss = criterion(logits.view(-1, len(vocab)), Y_tr[i:i+batch_size].view(-1))
            loss.backward()
        ...
        if (epoch + 1) % 10 == 0:
            print(f"Epoch {epoch+1} | Loss: {total_loss/len(Y_tr):.4f} | "
